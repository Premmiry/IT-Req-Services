...
export default function Fileupload({ onFilesChange, reqId, initialFiles = [] }: FileuploadProps) {
    ...
    useEffect(() => {
        const fetchExistingFiles = async () => {
            try {
                const response = await fetch(`http://localhost:1234/it-requests/images?req_id=${reqId}`);
                if (!response.ok) {
                            ...
                            >
                                <Typography variant="body2" color="primary">
                                    {file instanceof File ? (
                                        ...
                                        <a href={`http://localhost:1234/${file.file_path}`} target="_blank" rel="noopener noreferrer">
