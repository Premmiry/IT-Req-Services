...
const AssigneeDepSelector: React.FC<AssigneeDepSelectorProps> = ({
  ...
  const handleRemoveDepartment = async (id_req_dep: number) => {
    ...
    try {
      const response = await fetch(
        `${URLAPI}/assign_department_sub/${id_req_dep}`,
        {
          method: "DELETE",
        }
      );
      ...
      {assignedDepartments.length > 0 ? (
        <>
          {assignedDepartments.map((dept) => (
            <Tooltip key={dept.id_req_dep} title={dept.name_department} arrow>
              <Chip
                color="primary"
                variant="outlined"
                ...
                deleteIcon={
                  !readOnly && isITStaff ? <HighlightOffIcon sx={{ height: 16 }} /> : undefined
                }
                sx={{
                  ...
                  height: 20, // ปรับขนาดของ Chip ให้เล็กลง
                  fontSize: "0.69rem", // ปรับขนาดของข้อความใน Chip
                  borderRadius: '4px', // ปรับให้เป็นรูปสี่เหลี่ยม
                  maxWidth: 75, // ปรับความกว้างสูงสุดของ Chip
                  overflow: 'hidden',
        ...
        <Typography color="text.secondary" fontSize="0.777rem"></Typography>
      )}
      {!readOnly && isITStaff && (
        <IconButton
          ...
          sx={{
            ...
            width: 20, // ปรับขนาดของ IconButton ให้เล็กลง
            height: 20, // ปรับขนาดของ IconButton ให้เล็กลง
          }}
        >
                ...
                {filteredDepartments.map((department) => (
                  ...
                  >
                    <ListItemAvatar>
                      <Avatar
                        sx={{
                          bgcolor: randomColor(department.id_department_it),
                        }}
                      >
